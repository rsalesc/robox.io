...
def add_testcase(root: pathlib.Path, problem: DumpedProblem, testcase: Testcase):
  ...
  if not problem_path.is_file():
    console.print(f'[error]Problem [item]{problem.pretty_name()}[/item] not found.[/error]')
    return
  ...
  console.print(f'Added testcase [item]{i}[/item] to problem [item]{problem.pretty_name()}[/item].')
...
def remove_testcase(root: pathlib.Path, problem: DumpedProblem, i: int):
  ...
  if i >= len(problem.tests):
    console.print(f'[error]Testcase [item]{i}[/item] not found in problem [item]{problem.pretty_name()}[/item] metadata.[/error]')
  ...
  console.print(f'Removed testcase [item]{i}[/item] from problem [item]{problem.pretty_name()}[/item].')
...
def main(problem: Optional[str] = None):
  ...
  for dumped_problem in problems_to_hydrate:
    console.print(f'Hydrating problem [item]{dumped_problem.pretty_name()}[/item]...')
