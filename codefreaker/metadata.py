...
def _get_best_alias_from_candidates(alias: str, candidates: List[Tuple[pathlib.Path, DumpedProblem]]) -> Optional[Tuple[pathlib.Path, DumpedProblem]]:
  best_priority = 1e9
  best_candidates = []
...
def find_problem_path_by_alias(alias: str, root: Optional[pathlib.Path] = None) -> Optional[pathlib.Path]:
  if not root:
    root = pathlib.Path()
...
def find_problem_by_alias(alias: str, root: Optional[pathlib.Path] = None) -> Optional[DumpedProblem]:
...
def find_problem_by_anything(anything: str, root: Optional[pathlib.Path] = None) -> Optional[DumpedProblem]:
